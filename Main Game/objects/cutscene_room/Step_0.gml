	...
	case room_savannah_3:
		if (Flag_Get(FLAG_TYPE.STATIC,FLAG_STATIC.SCENE_004,false)==false){
			if (phase == 0) {
				...
				C_Wait(17, 20);
				C_CreateAnim(18, char_hippo, "x", 0, 0, char_hippo.x, char_player.x, 15, false);
				C_Wait(18, 10);
				...
				C_Execute(28, Dialog_Add_Ext, ["* Either way though, I think&  I can help Kaban a bit.", "{char_link 4}"]);
				C_Execute(28, Dialog_Add_Ext, ["* What will it be about,&  then?", "{char_link 2}{voice 4}{face 2}{face_emotion 4}"]);
				C_Execute(28, Dialog_Add_Ext, ["* About {color_text `yellow`}RUNNING{color_text `white`} it is!", "{char_link 4}"]);
				C_Execute(28, Dialog_Add_Ext, ["* I guess being able to&  run would sound nice.", "{char_link 2}{voice 4}{face 2}{face_emotion 3}"]);
